...
- Post-save validation now validates each expected render unit against page text and a padded
  extraction clip around the unit's final bbox.
...
- Validation failure diagnostics now include block ID, page, expected snippet, extracted clip
  snippet, source/final bboxes, font path, font size, overflow, and expansion state.
...
- Focused tests: `uv run pytest --no-cov tests/test_rendering.py` — `15 passed`.
...
- Full gate: `.\scripts\check.ps1` — `229 passed, 1 skipped`, coverage `88.62%`.
