...
## PDFTranslate project guidance

Apply these patterns within the repository's actual Windows-first Python 3.12 CLI architecture:

- keep Typer commands thin and delegate PDF, translation, rendering, OCR, and batch behavior to
  typed Python functions or focused services;
- model stable boundaries with `Protocol` only when multiple implementations or a deterministic
  test double justify the abstraction, especially for translators and subprocess-backed adapters;
- compose explicit pipeline stages rather than building a deep inheritance hierarchy;
- keep PyMuPDF, model/tokenizer, CUDA, and OCR-specific types inside their adapters;
- use Pydantic models at configuration or serialization boundaries, not for every internal value;
- make filesystem ownership, cache/workspace paths, temporary output, and atomic publication
  explicit;
- inject heavyweight model loaders and external-process runners so unit tests never download
  models, require CUDA, or invoke OCR tools;
- do not introduce generic web-style controller/service/repository layers merely because examples
  below use them; add a layer only when it owns a real PDFTranslate responsibility;
- preserve source-PDF immutability and keep output validation separate from translation logic;
- prefer small synchronous functions unless concurrency provides a measured benefit for batch or
  model execution.
